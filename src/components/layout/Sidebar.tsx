...
export const Sidebar: React.FC<SidebarProps> = ({
  ...
  const navItems = [
    ...
    { id: "github", label: "GitHub Analyzer", icon: Github, badge: null },
    ...
    >
      ...
      <div>
        <div className="p-4 border-b border-slate-200 dark:border-slate-800 flex items-center justify-between">
          <div className="flex items-center gap-3 cursor-pointer" title="Go to Public Website" onClick={() => handleSelect("landing")}>
            <img
              src={logoImg}
        ...
        <div className="mx-3 my-3 p-3.5 rounded-2xl bg-slate-100 dark:bg-slate-800/40 border border-slate-200 dark:border-slate-800">
          <div className="flex items-center justify-between text-xs font-semibold text-slate-700 dark:text-slate-300 mb-1.5">
            <span className="flex items-center gap-1 text-[11px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider">
              <Sparkles className="w-3.5 h-3.5 text-emerald-500" /> Career Readiness
            </span>
            <span className="text-emerald-600 dark:text-emerald-400 font-bold">{user.careerReadiness || 0}%</span>
          </div>
          <div className="w-full bg-slate-200 dark:bg-slate-800 h-2 rounded-full overflow-hidden">
            <div
              className="bg-emerald-500 h-full rounded-full transition-all duration-500"
              style={{ width: `${user.careerReadiness || 0}%` }}
            />
          </div>
          <div className="text-[11px] text-slate-500 dark:text-slate-400 mt-2 truncate font-semibold">
            {user.targetCareer || "Software Engineer"}
          </div>
        </div>

        {/* Primary Navigation */}
        <nav className="px-3 space-y-1 overflow-y-auto max-h-[calc(100vh-300px)] scrollbar-none py-1">
          ...
          <div className="pt-3 px-3 py-1 text-[10px] font-bold text-slate-400 dark:text-slate-500 uppercase tracking-wider">Account</div>
        ...
        >
          <div className="flex items-center gap-3">
            ...
            <div className="truncate max-w-[130px]">
              <div className="text-sm font-semibold text-slate-900 dark:text-white truncate">{user.fullName || "Candidate"}</div>
              <div className="text-xs text-slate-500 dark:text-slate-400 truncate">{user.degree || "CS Student"}</div>
            </div>
          </div>
          <button
            onClick={(e) => {
